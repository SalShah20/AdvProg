...
public class IceCreamParlorAL
{
    ...
    {
        // change code here
        int min = inventory.get(1).getCalories();
        for (IceCream i : inventory) {
            min = Math.min(min, i.getCalories());
        }
        return min;
    }
    ...
    public ArrayList<IceCream> locateLowCalIceCreams()
    {
        ...
        for (int i = 0; i < inventory.size(); i++) {
            if (inventory.get(i).getCalories() == minCal) {
                lowCalVarieties.add(index, inventory.get(i));
                index++;
            }
        }
    ...
    public ArrayList<IceCream> locateIngredientMatches(String ingredient)
    {
        ...
        // add code here
        for (IceCream i : inventory) {
            if (i.isIngredientFound(ingredient)) {
                counter++;
                match.add(index, i);
                index++;
            }
